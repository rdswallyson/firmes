  ...
  useEffect(() => {
    fetch("/api/superadmin/dashboard")
      ...
      .then(d => {
        if (d.error) {
          setError(d.error);
          if (d.error === "Não autorizado") router.push("/superadmin");
        } else {
          setData(d);
        }
      })
    ...
    <div style={{ minHeight: "100vh", background: "#F5F0EB" }}>
      ...
      <div style={{ padding: "24px", maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 24, display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <Link href="/superadmin/metricas" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 18px", background: "#fff", color: NAVY, borderRadius: 10, fontSize: 14, fontWeight: 700, textDecoration: "none", border: "1px solid #E5E7EB" }}>
            <TrendingUp size={16} /> Metricas
          </Link>
        </div>
      </div>
    </div>
